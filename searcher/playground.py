...
async def try_except_query_data(query_string, dest, limit, page, rqa=5):
    try:
        x = await get_query_data(query_string=query_string, dest=dest, limit=limit, page=page, rqa=rqa, timeout=3)
    except ValueError:
        x = {"products": []}
    return x
...
async def get_r_data(r, city, date):
    while True:
        try:
            full_res = []
            tasks = [asyncio.create_task(try_except_query_data(query_string=r.query, dest=city.dest, limit=300, page=i, rqa=3)) for i in range(1,4)]
